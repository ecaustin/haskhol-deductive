...
convAC acsuite = Conv $ \ tm ->
    ruleEQT_ELIM $ runConv (convPURE_REWRITE [toHThm acsuite, thmEQ_REFL_T]) tm
...
thmIMP_CONJ = cacheProof "thmIMP_CONJ" ctxtTheorems $
    prove [txt| p /\ q ==> r <=> p ==> q ==> r |] tacITAUT
...
thmSWAP_FORALL = cacheProof "thmSWAP_FORALL" ctxtTheorems $
    prove [txt| !P:A->B->bool. (!x y. P x y) <=> (!y x. P x y) |] tacITAUT
...
thmRIGHT_EXISTS_AND = cacheProof "thmRIGHT_EXISTS_AND" ctxtTheorems $
    prove [txt| !P Q. (?x:A. P /\ Q x) <=> P /\ (?x:A. Q x) |] tacITAUT
...
thmEXISTS_UNIQUE_REFL = cacheProof "thmEXISTS_UNIQUE_REFL" ctxtTheorems $
    prove [txt| !a:A. ?!x. x = a |] $
      ...
      [ tacEXISTS [txt| a:A |], tacASM_REWRITE_NIL ] `_THEN`
      tacREFL
...
thmEXISTS_UNIQUE_ALT = cacheProof "thmEXISTS_UNIQUE_ALT" ctxtTheorems .
  prove [txt| !P:A->bool. (?!x. P x) <=> (?x. !y. P y <=> (x = y)) |] $
    tacGEN `_THEN` tacREWRITE [thmEXISTS_UNIQUE] `_THEN` tacEQ `_THENL`
    [ _DISCH_THEN (_CONJUNCTS_THEN2 (tacX_CHOOSE [txt|x:A|]) tacASSUME) `_THEN`
      tacEXISTS [txt| x:A |] `_THEN` tacGEN `_THEN` tacEQ `_THENL`
      [ tacDISCH `_THEN` _FIRST_ASSUM tacMATCH_MP `_THEN` tacASM_REWRITE_NIL
      , _DISCH_THEN (\ th -> tacSUBST1 (ruleSYM th)) `_THEN` 
        _FIRST_ASSUM tacMATCH_ACCEPT
      ]
    , _DISCH_THEN (tacX_CHOOSE [txt| x:A |]) `_THEN`
      tacASM_REWRITE [ruleGSYM thmEXISTS_REFL] `_THEN` 
      _REPEAT tacGEN `_THEN`
      _DISCH_THEN (_CONJUNCTS_THEN (\ th -> tacSUBST1 (ruleSYM th))) `_THEN` 
      tacREFL
    ]

thmUNWIND1 :: TheoremsCtxt thry => HOL cls thry HOLThm
thmUNWIND1 = cacheProof "thmUNWIND1" ctxtTheorems $
    prove [txt| !P (a:A). (?x. a = x /\ P x) <=> P a |] $
      _REPEAT tacGEN `_THEN` tacEQ `_THENL`
      [ _DISCH_THEN (_CHOOSE_THEN (_CONJUNCTS_THEN2 tacSUBST1 tacACCEPT))
      , tacDISCH `_THEN` tacEXISTS [txt| a:A |] `_THEN`
